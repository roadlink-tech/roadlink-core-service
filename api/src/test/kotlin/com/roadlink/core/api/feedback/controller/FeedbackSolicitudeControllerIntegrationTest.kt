...
class FeedbackSolicitudeControllerIntegrationTest : BaseControllerTest() {
    ...
    fun `when create a valid feedback solicitude, the it must work ok`() {
        ...
        val response = mockMvc.perform(
            MockMvcRequestBuilders.post("/core-service/users/${george.id}/feedback_solicitudes").content(
                """{
    ...
    fun `when list pending feedback solicitudes, the it must be retrieved`() {
        ...
        // When
        val response = mockMvc.perform(
            MockMvcRequestBuilders.get("/core-service/users/${george.id}/feedback_solicitudes?status=PENDING")
    ...
    fun `when complete a feedback solicitude successfully, then it must work ok`() {
        ...
        // When
        val response = mockMvc.perform(
            MockMvcRequestBuilders.put("/core-service/users/${george.id}/feedback_solicitudes/$feedbackSolicitudeId/complete")
                .content(
                    """{
                        "comment":"ok",
                        "rating":"4"
